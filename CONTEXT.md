...
**Layout**:
The single module (`Art4\LegacyTodo\Layout`) that owns the full page chrome, the output-escaping surface, and error/abort responses — the one `<html>` scaffold, `<head>`, per-page `<title>`, and closing tags via `header(?string $title = null)`/`footer()`, plus `text()`/`attr()` escaping used by every page that renders markup. Every rendered page (the six page handlers and `index.php`) composes `header($title) . content . footer()` through this one seam; the per-page title is escaped inside via `text()` (ADR-0014). Denied and aborted requests compose the same chrome through `errorPage(int $status, string $message)`, which sets the real wire status (404 not-found, 403 denials) — no page bypasses the chrome on an error path (ADR-0016). Constructed by Bootstrap with the site name and `Auth` (ADR-0009).
...
**Owner**:
The User a Todo belongs to. The Owner or an Administrator may edit, archive, or delete a Todo's comments; anyone else is refused permission for those actions. Commenting on or assigning a Todo is open to every authenticated user — the collaboration surface, not an ownership decision (ADR-0015).
